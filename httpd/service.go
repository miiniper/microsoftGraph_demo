...
func (s *Service) Start() error {
	...
	go func() {
		...
		if err != nil {
			Loges.Error("httpd serve error", zap.Error(err))
		}
	}()
	Loges.Info("httpd service started", zap.String("listen", s.addr))
...
func (s *Service) accessLog(inner http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if dur <= 1e3 {
			Loges.Info("http access", zap.String("method", r.Method), zap.String("uri", r.RequestURI), zap.Int64("time(us)", dur), zap.String("RemoteClentIP", remoteIP))
		} else {
			Loges.Info("http access", zap.String("method", r.Method), zap.String("uri", r.RequestURI), zap.Int64("time(ms)", dur/1e3), zap.String("RemoteClentIP", remoteIP))
		}
	})
}
